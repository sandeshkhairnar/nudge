...
import { Sparkles, Clock, CheckCircle2, X, Loader2, RefreshCw, Zap, Settings2, Plus, Info } from "lucide-react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import { useNudgeSubscription } from "@/hooks/useNudgeSubscription";
...
export function NudgesClient({ initialNudges }: { initialNudges: Nudge[] }) {
  const supabase = createClient();
  const workspace = useWorkspaceStore((s) => s.workspace);
  const queryClient = useQueryClient();
  
  // Realtime subscription
  useNudgeSubscription(workspace?.id);
  ...
  // TanStack Query for Nudges
  const { data: nudges = initialNudges, isLoading } = useQuery({
    queryKey: ["nudges", workspace?.id],
    queryFn: async () => {
    ...
    initialData: initialNudges,
  });

  // Mutation for dismissing nudges
  const dismissMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("nudges").update({ dismissed: true }).eq("id", id);
      if (error) throw error;
    },
    onMutate: async (id: string) => {
      await queryClient.cancelQueries({ queryKey: ["nudges", workspace?.id] });
      const previous = queryClient.getQueryData(["nudges", workspace?.id]);
      queryClient.setQueryData(["nudges", workspace?.id], (old: Nudge[] | undefined) => {
        if (!old) return old;
    ...
    onError: (err, id, context) => {
      if (context?.previous) {
        queryClient.setQueryData(["nudges", workspace?.id], context.previous);
      }
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ["nudges", workspace?.id] });
    }
  });
  ...
  const saveTimes = async (times: string[]) => {
    if (!workspace?.id) return;
    await supabase.from("workspaces").update({ nudge_check_times: times }).eq("id", workspace.id);
  };
  ...
  if (isLoading && nudges.length === 0) return (
    ...
    <div className="w-full flex flex-col font-sans bg-transparent pb-12">
      ...
      <div className="pb-6 mb-6 w-full flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => queryClient.invalidateQueries({ queryKey: ["nudges", workspace?.id] })}
                  ...
                  <div className="flex-1 min-w-0 w-full">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="px-4 py-1.5 bg-white text-gray-600 border border-gray-200 rounded-lg text-[12px] font-semibold cursor-pointer hover:bg-gray-50 hover:text-gray-900 transition-colors shadow-sm disabled:opacity-50"
                      >
                        Dismiss
                      </button>
                    </div>
